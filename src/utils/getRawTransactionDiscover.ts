...
export const createManagerForwardCall = async ({
  ...
  console.log('=====getRawTransaction managerForwardCall handleManagerForwardCall', res);

  res.args = Buffer.from(AElf.utils.uint8ArrayToHex(res.args), 'hex').toString('base64');
  console.log('=====getRawTransaction managerForwardCall res.args', res.args);

  const methods = await getContractMethods(instance, caContractAddress);
  console.log('=====getRawTransaction managerForwardCall methods', methods);

  const protoInputType = methods['ManagerForwardCall'];
  console.log('=====getRawTransaction managerForwardCall protoInputType', protoInputType);
  ...
  console.log('=====getRawTransaction managerForwardCall message', message);
...
export const getRawTransactionDiscover = async ({
  ...
  try {
    ...
    console.log('=====getRawTransaction managerForwardCall', caHash);
